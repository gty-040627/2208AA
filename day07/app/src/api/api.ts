...
//用户搜索
export const qsueryUserListSearch = () => http.get('/users').then(res => res.data)
//用户编辑回显
export const qsueryUserListLook = (id: number) => http.get(`/users?${id}`).then(res => res.data)
//用户编辑确认
export const qsueryUserListLookList = (data: TS.MeitForm) => http.put(`users/${data.id}`, data).then(res => res.data)
//用户删除
export const qsueryUserListDel = (id: number) => http.delete(`/users/${id}`).then(res => res.data)
//用户分配角色回填
export const qsueryUserListLook2 = () => http.get('/roles').then(res => res.data)
...
//角色列表删除
export const queryRolesDel = (id: number) => http.delete(`/roles/${id}`).then(res => res.data)
//角色列表编辑
...
//树状图添加
export const queryRightListAdd = (id: number) => http.post(`roles/${id}/rights`, id).then(res => res.data)
//树状图删除
export const queryRightListDel = (id: number) => http.delete(`roles/${id}/rights/${id}`).then(res => res.data)
//角色列表权限列表 首页
...
//编辑回填
export const queryGoodsListLook = (goods_id: number) => http.get(`/goods/${goods_id}`).then(res => res.data)
//编辑
